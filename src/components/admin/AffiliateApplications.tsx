...
export default function AffiliateApplications({ initialApplications }: { initialApplications: AffiliateApplicationWithStats[] }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch('/api/affiliates/pending', { cache: 'no-store' });
        if (!res.ok) return;
        const payload = await res.json();
        if (!cancelled && Array.isArray(payload?.applications)) {
          setApplications(payload.applications as AffiliateApplicationWithStats[]);
        }
      } catch {
        // ignore
      }
